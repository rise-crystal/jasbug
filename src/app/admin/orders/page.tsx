  ...
  return (
    <main className="min-h-screen bg-gray-950 relative overflow-hidden py-8 px-4">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-center py-6 px-6 rounded-t-2xl mb-6 shadow-2xl shadow-blue-500/30">
          <div className="flex items-center justify-center gap-3 mb-2">
            <h1 className="text-4xl font-black text-white">📊 ADMIN - SEMUA ORDERS</h1>
            <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">ADMIN ONLY</span>
          </div>
          <p className="text-blue-100 text-sm">Kelola semua pesanan</p>
        </div>
        ...
        <div className="bg-gray-900 border-2 border-blue-500/50 p-4 mb-6">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            <div className="flex items-center gap-4">
              <div className="bg-blue-900/50 border border-blue-500/30 rounded-lg px-4 py-2">
                <span className="text-2xl font-black text-blue-400">{orders.length}</span>
                <span className="text-sm text-gray-400 ml-2">Total Orders</span>
              </div>
              {selectedOrders.size > 0 && (
                <div className="bg-orange-900/50 border border-orange-500/30 rounded-lg px-4 py-2">
                  <span className="text-2xl font-black text-orange-400">{selectedOrders.size}</span>
                  <span className="text-sm text-gray-400 ml-2">Dipilih</span>
                </div>
              )}
            </div>
            <div className="flex gap-2 flex-wrap">
              {selectedOrders.size > 0 && (
                <button onClick={handleDeleteSelected} disabled={deleting} className="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 disabled:from-gray-600 disabled:to-gray-700 text-white font-bold py-2 px-4 rounded-lg transition disabled:shadow-none">
                  {deleting ? '⏳ Menghapus...' : `🗑️ Hapus (${selectedOrders.size})`}
                </button>
              )}
              <a href="/admin" className="bg-gray-800 hover:bg-gray-700 border-2 border-purple-500/50 text-purple-500 font-bold py-2 px-4 rounded-lg transition">🔐 Verifikasi</a>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              {paginatedOrders.map((order) => {
                ...
                return (
                  <div key={order.id} className={`bg-gray-800 border-2 rounded-xl p-5 transition-all ${isSelected ? 'border-orange-500 shadow-lg shadow-orange-500/20' : 'border-blue-500/30 hover:border-blue-500/70'}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-3 flex-wrap">
                          <span className="font-mono text-xs text-blue-400 bg-gray-900 px-3 py-1.5 rounded-lg border border-blue-500/30 font-bold">{order.custom_id || order.id}</span>
                        ...
                        {product && (
                          <div className="flex items-center gap-3 mb-3 p-3 bg-gray-900 rounded-lg border border-blue-500/20">
                            <img src={product.image} alt={product.name} className="w-10 h-10 object-contain" />
                            <div><p className="text-sm font-bold text-gray-300">{product.name}</p></div>
                          </div>
                        )}

                        <div className="flex flex-wrap items-center gap-4">
                          <p className="text-gray-300 text-sm">📱 <span className="font-mono text-orange-400">{order.phone_number}</span></p>
                          {order.payment_amount && <p className="text-white font-bold text-sm">💰 Rp {order.payment_amount.toLocaleString('id-ID')}</p>}
